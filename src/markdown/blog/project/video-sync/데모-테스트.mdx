...
## 데모 애플리케이션(클라이언트)
최초엔 클라이언트 애플리케이션은 단순히 시간을 응답받아 `performance.now()`를 통해 시간이 흘러가는 `clock` 함수를 만들었다. 그렇게 여러 개의 클라이언트에서 각각 `clock`이 반환하는 값을 띄워놓고 시간이 얼마나 잘 맞아떨어지나 눈으로 비교하였다. 하지만 ms단위로 흘러가는 시계의 텍스트를 번갈아보며 이것이 서로 일치하는지 판별하기란 영 쉽지 않았다. 스크린샷을 찍어 비교하는 방법도 있었으나 수고로움이 상당했다

그래서 **애니메이션을 통해 시간 동기화 정도를 판별**하는 방법을 생각하였다. 인간의 눈이 텍스트가 `"35:19"`에서 `"35:59"`로 변하는 것을 인지하는 것보단 빨간색 상자가 파란색 상자로 변하는 것을 더 빠르게 인지할 수 있다는 점에서 착안하였다
...
- 색상의 변화를 극단적으로 느끼기 위해 인접한 색은 보색관계가 되도록 색상을 배치하였다
- 애니메이션은 `requestAnimationFrame`을 통해 매 렌더링 프레임마다 시간을 기반으로 인덱스를 추정하여 해당하는 색상을 draw하였다

---

## 테스트 결과
**매우 잘 동작했다!** 60fps의 외장 모니터에서도, 120fps의 내장 모니터에서도, Windows 데스크탑에서 동시에 애플리케이션을 실행하여도 오차를 느낄 수 없었다. 심지어 2시간이 지난 뒤에도 싱크는 계속해서 맞았고, 한참 시간이 흐른 뒤에 애플리케이션을 추가로 실행하여 애니메이션을 재생하여도 동기화가 매우 잘 되었다:

<video autoPlay loop muted playsInline width="768">
  <source src="/markdown/blog/project/video-sync/demo_sync.mp4" type="video/mp4" />
</video>

독립된 환경에서 테스트하기 위해 두 개의 컴퓨터 각각에서 데모 애플리케이션을 실행하였다:
- Windows 데스크탑(LAN 연결):
...
이렇게 하면 체감 딜레이는 더 줄어들었다. 하지만 재요청시에도 값이 업데이트 되지 않는 등, API 이해가 충분하지 않은 상태에서 사용하다 보니 버그가 눈에 띄었다. 아무래도 API에 대한 이해를 먼저 한 뒤에 적용해야할 것 같다

### (임시 방편) 여러 번 요청하기
딜레이가 긴 경우는 보통 첫 요청을 보낼 때였다. 첫 요청 이후 여러번 요청할 경우 응답 시간이 줄어들었고, **딜레이 또한 줄어들어 싱크가 맞았다**. 다른 컴퓨터끼리 싱크가 맞지 않는 것 또한 여러번 요청하면 싱크가 맞게 되었다. 그래서 임시 방편이지만 동기화 작업은 응답 하나만으로 결정하지않고 **여러 응답중 RTT 값이 가장 작은 응답을 선택**하는 방법으로 싱크를 맞추려 한다. 근본적인 해결 방법은 아니지만 당장은 잘 작동하기에 선택하였다. 추후 근본적인 해결방법을 찾아내면 해당 방법을 적용할 것이다
